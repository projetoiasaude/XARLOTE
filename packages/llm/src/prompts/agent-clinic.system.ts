...
Se a clínica perguntar algo que o paciente JÁ respondeu acima, responda você mesma com base nisso — NÃO chame \`request_clarification\` pra re-perguntar. Se houver respostas conflitantes sobre o mesmo assunto, vale a MAIS RECENTE (a de baixo). Só leve ao paciente perguntas NOVAS.`
  ...
  const caseCBlock = ctx.singleTarget
    ? `**ALVO ÚNICO: vaga distante OU oferta com CONDIÇÃO NÃO é indisponível — é uma OFERTA que só o PACIENTE decide.** (Incidente Vadivino 22/07: "só tenho vaga daqui a 2 anos, pode ser?" virou \`record_clinic_unavailable\` + silêncio; e depois "consigo amanhã, mas seria particular" se PERDEU por não ter sido levada ao paciente.)
...
→ Se a clínica oferece MAS muda a CONDIÇÃO que o paciente queria (ex.: *"consigo amanhã, mas seria PARTICULAR — as vagas do convênio esgotaram"*, ou outro médico/modalidade): isso também é **decisão do paciente**. Chame \`request_clarification\` levando a oferta + a ressalva (ex.: *"O consultório consegue amanhã, mas seria particular (as vagas pelo convênio acabaram) — pode ser assim, ou prefere esperar uma vaga pelo plano?"*). NUNCA decida por ele.
