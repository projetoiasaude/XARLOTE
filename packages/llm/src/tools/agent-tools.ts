...
export const agentPharmacyTools: ToolDefinition[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Registra preço, frete, prazo, forma de pagamento E O PRODUTO que a farmácia disse que tem. Chame de novo pra ATUALIZAR (ex.: o frete chegou depois).',
      parameters: {
        type: 'object',
        properties: {
          subtotal: { type: 'number' },
          delivery_fee: { type: 'number', description: 'Frete em reais. 0 SÓ se a farmácia disse que é grátis. OMITA se ela ainda não falou do frete (nunca 0 como "não sei").' },
          total: { type: 'number' },
          eta_minutes: { type: 'number' },
          payment_methods: {
            type: 'array',
            items: { type: 'string', enum: ['pix', 'cartao', 'dinheiro'] },
            description: 'SÓ o que a farmácia DISSE aceitar. Se ela não falou de pagamento, OMITA (nunca chute "pix").',
          },
          pix_key: { type: 'string' },
          payment_link: { type: 'string' },
          notes: { type: 'string' },
          product_as_quoted: { type: 'string', description: 'O produto EXATAMENTE como a farmácia nomeou (ex.: "Venaflon", "Daflon Flex 1000mg 30 envelopes", "genérico da dipirona"). Se ela só deu o preço sem nomear, OMITA — nunca repita o nome do pedido como se ela tivesse confirmado.' },
          is_substitute: { type: 'boolean', description: 'true se o que ela tem é OUTRO produto (similar/genérico/concorrente/outra apresentação da mesma marca NÃO conta como substituto). false se ela confirmou ter exatamente o pedido. Omita se não dá pra saber.' },
          presentation: { type: 'string', description: 'Apresentação dita por ela (ex.: "30 comprimidos", "caixa com 20", "30 envelopes"). Omita se não disse.' },
        },
        required: ['total'],
      },
    },
