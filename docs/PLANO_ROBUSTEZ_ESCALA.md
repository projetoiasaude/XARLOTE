...
- [x] **F1.A3 — Consolidação de cotação durável (farmácia + clínica).** _Por quê:_ `setTimeout`+`Set` em memória (`quote-consolidation.ts:6`); só clínica tem rescue. _DoD:_ estado em DB + dispatcher que resgata pendentes (estender o `consultation-dispatcher` pra farmácia); restart não perde consolidação; teste. **M** — ✅ **(2026-06-08):** novo `rescueOrphanedPharmacyQuotes()` — scan de pedidos presos em 'quoting' além de `PHARMACY_RESCUE_WINDOW_MIN` (default 10min) chamando `consolidateQuotes` (já idempotente: transição atômica quoting→quoted). Rodando no `consultation-dispatcher` (worker, 30s) — agora resgata farmácia + clínica. Restart não perde mais a consolidação (estado já vivia no DB: status + status_3/5min + created_at).
- [~] **F1.A4 — Idempotência ponta-a-ponta.** _Por quê:_ replay de webhook ou retry pode duplicar pedido/mensagem. _DoD:_ chave de idempotência em sends e tool-calls que criam recursos; reprocessar o mesmo evento é no-op; teste de replay. **M** — **🔄 (2026-06-08, já em vigor):** webhook é idempotente por `messageid` (`webhook_events` + skip em 23505) → replay de inbound não reprocessa. `start_pharmacy_order` tem guarda de IDEMPOTÊNCIA (`tool-executor.ts:257`): com pedido ativo, re-chamar é no-op (só re-reporta status, não duplica). **Follow-up:** chave de idempotência nos ENVIOS WhatsApp (retry da fila pode, em teoria, duplicar msg se o ack se perder) — uazapi não expõe idempotency key nativa; avaliar dedupe por hash (conversa+texto+janela).
